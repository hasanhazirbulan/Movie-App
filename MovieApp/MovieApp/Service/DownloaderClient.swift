...
class DownloaderClient {
    ...
    func filmDetayiniIndir(imdbId : String, completion: @escaping (Result<FilmDetails,DownloaderError>) -> Void) {
        ...
        URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            guard let gelenFilmDetayi = try? JSONDecoder().decode(FilmDetails.self, from: data) else {
                return completion(.failure(.veriIslenemedi))
            }
